...
----------------Ubuntu Raspberry Pi 4B----------------------------------
# Getting The RGBD Camera to work on RBPi (Works)
...
# Installing Realsense Stuff on Raspberry Pi
...
# Connecting RBPi to Pixhawk and establish ROS2 connection:
...
ros2 topic list
```
6. for aa successful communication link establishment, you should see lots of topics starting with "/fmu/.........". If you dont see this, check that your "ROS_DOMAIN_ID" in bash.rc onPi and  UXRCE_DDS_DOM_ID parameter on FC are the same (preferably 0). If still not work, also check local host


# Sending/reading topics from FC
...
2. TEST: After RBPi has been added, changed following settings (Test of RBPi can arm/disarm drone):
	...
	4. (MAVLink or uxrce???????)Follow [this page](https://docs.px4.io/main/en/ros2/offboard_control.html) to test drone in offboard mode. The programme starts sending setpoints, enters offboard mode, arms, ascends to 5 metres, and waits
		1. NOTE: 

...
# To change motor speed in ros, use: "ros2 param set /motor_control_node motor_speed 0.1"
...
class MotorControlNode(Node):
    def __init__(self):
        ...
        self.control_publisher = self.create_publisher(ActuatorControl, "/fmu/in/actuator_controls", qos_profile)
        ...
        # Declare parameters for dynamic motor speed control
        self.declare_parameter('motor_speed', 0.05)  # Default speed of 0.05
    ...
    def set_motor_values(self, motor_speed):
        """ Set the same value for all motors (normalized from -1.0 to 1.0) """
        # Update all motor control values to the new speed
        self.control_msg.control[0] = motor_speed
        self.control_msg.control[1] = motor_speed
        self.control_msg.control[2] = motor_speed
        self.control_msg.control[3] = motor_speed
    ...
    def control_loop_callback(self):
        ...
        offboard_mode_msg.thrust_and_torque = False  # No thrust and torque control
        offboard_mode_msg.direct_actuator = True  # Enable direct actuator control

        self.offboard_mode_publisher.publish(offboard_mode_msg)

        # Get the motor speed from the parameter server
        motor_speed = self.get_parameter('motor_speed').value
        self.set_motor_values(motor_speed)
        ...
        if (self.nav_state == VehicleStatus.NAVIGATION_STATE_OFFBOARD and 
            ...
            self.get_logger().info(f'Motor commands sent with speed: {motor_speed}')
